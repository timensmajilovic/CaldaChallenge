...
VALUES
  (uuid_generate_v4(), 'anze.novak@email.com', 'Anze Novak'),
  (uuid_generate_v4(), 'mojca.doe@email.com', 'Mojca Doe'),
  (uuid_generate_v4(), 'john.who@email.com', 'John Who');
...
VALUES
  ('Office Keyboard', 'Basic keyboard for PC', 10.99, 100),
  ('Office Mouse', 'Basic mouse for PC', 15.50, 50),
  ('Office Headset', 'Basic headset for audio with built-in microphone', 7.25, 200),
  ('Mousepad', 'Full-size mousepad', 20.00, 30),
  ('Wrist rest', 'Wrist rest for PC mouse', 5.75, 150);

-- ===============================
-- ORDERS
-- ===============================
INSERT INTO orders(user_id, recipient_name, shipping_address)
SELECT id, full_name, '123 Main Street' FROM users WHERE full_name = 'Anze Novak';

INSERT INTO orders(user_id, recipient_name, shipping_address)
SELECT id, full_name, '456 Elm Street' FROM users WHERE full_name = 'Mojca Doe';

-- ===============================
-- ORDER ITEMS
-- ===============================
-- Order 1 for Anze Novak
INSERT INTO order_items(order_id, item_id, quantity, price_at_purchase)
SELECT o.id, i.id, 2, i.price
FROM orders o
JOIN items i ON i.name IN ('Office Keyboard', 'Office Mouse')
WHERE o.user_id = (SELECT id FROM users WHERE full_name = 'Anze Novak');

-- Order 2 for Mojca Doe
INSERT INTO order_items(order_id, item_id, quantity, price_at_purchase)
SELECT o.id, i.id, 1, i.price
FROM orders o
JOIN items i ON i.name IN ('Mousepad', 'Wrist rest')
WHERE o.user_id = (SELECT id FROM users WHERE full_name = 'Mojca Doe');
